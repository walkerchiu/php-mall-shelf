...
class StockObserver
{
    ...
    {
        if (!empty($entity->inventory) &&
            !empty($entity->quantity) &&
            $entity->inventory < $entity->quantity)
            return false;

        if (config('wk-core.class.mall-shelf.stock')::where('id', '<>', $entity->id)
    ...
    {
        if ( config('wk-mall-shelf.onoff.mall-wishlist') && !empty(config('wk-core.class.mall-wishlist.item')) ) {
            $entity->wishlist()->delete();
        }

        if ($entity->isForceDeleting()) {
            $entity->langs->withTrashed()->forceDelete();
            if ( config('wk-mall-shelf.onoff.morph-image') && !empty(config('wk-core.class.morph-image.image')) ) {
            ...
            if ( config('wk-mall-shelf.onoff.mall-cart') && !empty(config('wk-core.class.mall-cart.item')) ) {
